...
USER_AGENT = "Mission1000-DataEngine/6.2-WTA-H2H-FINAL"
...
def _parse_tennisratio_h2h(page, player1, player2, url):
    """
    Final conservative parser for TennisRatio.

    Only accepts explicit natural-language H2H sentences of the form:
    "<A> and <B> have met 4 times ... with <Leader> leading 4-0."

    If that exact relationship is not present and internally consistent,
    return None. No guessing from unrelated numbers on the page.
    """
    text = _html_text(page)
    p1 = normalize_player_name(player1)
    p2 = normalize_player_name(player2)

    # Match the observed prose while keeping the middle text flexible.
    # Example:
    # "Diana Shnaider and Jessica Pegula have met 4 times in professional
    # competition, with Jessica Pegula leading 4-0."
    rx = re.compile(
        r"(?P<a>[A-Za-zÀ-ÖØ-öø-ÿ .'\-]+?)\s+and\s+"
        r"(?P<b>[A-Za-zÀ-ÖØ-öø-ÿ .'\-]+?)\s+have\s+met\s+"
        r"(?P<meetings>\d+)\s+times\b"
        r"(?P<middle>.{0,260}?)"
        r"with\s+(?P<leader>[A-Za-zÀ-ÖØ-öø-ÿ .'\-]+?)\s+leading\s+"
        r"(?P<leadwins>\d+)\s*[-:–]\s*(?P<trailwins>\d+)",
        flags=re.I | re.S,
    )
    ...
    for m in rx.finditer(text):
        a = normalize_player_name(m.group("a"))
        b = normalize_player_name(m.group("b"))
        leader = normalize_player_name(m.group("leader"))

        if {a.casefold(), b.casefold()} != {p1.casefold(), p2.casefold()}:
            continue

        meetings = int(m.group("meetings"))
        lead_wins = int(m.group("leadwins"))
        trail_wins = int(m.group("trailwins"))
        ...
        best = {
            ...
            "lastMeeting": None,
            "recentMeetings": [],
            "source": "TennisRatio H2H (validated prose)",
            "sourceUrl": url,
        }
        break

    if not best:
        print(f"TennisRatio FINAL: kein validierter H2H für {p1} vs {p2}")
        return None

    print(
        f"TennisRatio FINAL OK: {p1} vs {p2} -> "
        f"{best['wins1']}:{best['wins2']} | meetings={best['meetings']}"
    )
    return best
    ...
    for url, parser in candidates:
        try:
            ...
            if record and int(record.get("meetings") or 0) > 0:
                print(
                    f"WTA H2H FINAL OK: {player1} vs {player2} -> "
...
def supplement_current_wta_h2h_web(h2h):
    ...
    for m in matches:
        ...
        if ext_n != int(external.get("wins1") or 0) + int(external.get("wins2") or 0):
            print("WTA H2H FINAL reject: Meeting-Summe inkonsistent", p1, "vs", p2)
            continue

        if ext_n > 30:
            print("WTA H2H FINAL reject: ungewöhnlich viele Meetings", p1, "vs", p2, ext_n)
            continue
    ...
    print("WTA H2H FINAL ergänzt/ersetzt:", changed)
    return h2h
...
def main():
    print("=" * 68)
    print("MISSION 1000 DATA ENGINE v6.2 FINAL SAFE H2H")
    print("RUNNING FILE: collector/fetch_intelligence_v6_2_final_safe_h2h.py")
    print("=" * 68)
    ...
    if peg_shn:
        ...
        print(
            "H2H FINAL CHECK Pegula/Shnaider:",
            f"Pegula {peg_wins}:{shn_wins} Shnaider",
            "| Meetings:", meetings,
            "| Source:", peg_shn.get("source")
        )
    else:
        print("H2H FINAL CHECK Pegula/Shnaider: NICHT GEFUNDEN")
    ...
    save_json(SOURCE / "rankings_fallback.json", {
        "generatedAt": now,
        "source": "Mission 1000 Data Engine v6.2 - latest observed match rankings",
        "players": ranking_fallback,
    })

    save_json(SOURCE / "players.json", {
        "generatedAt": now,
        "source": "Mission 1000 Data Engine v6.2",
        "players": players,
    })
    ...
    print("DATA ENGINE v6.2 FINAL SAFE H2H: OK")
