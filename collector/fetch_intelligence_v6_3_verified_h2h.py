...
USER_AGENT = "Mission1000-DataEngine/6.3-VERIFIED-H2H"
...
def _parse_tennisratio_h2h(page, player1, player2, url):
    """
    VERIFIED parser for TennisRatio H2H prose.

    It never scans generic page numbers. It anchors on the EXACT two player
    names supplied by the current match, then reads only the explicit sentence:

      "<A> and <B> have met N times ... with <Leader> leading X-Y."

    The result is accepted only when X + Y == N and Leader is one of A/B.
    """
    text = _html_text(page)
    p1 = normalize_player_name(player1)
    p2 = normalize_player_name(player2)

    # Escape the CURRENT match players themselves. This prevents strings like
    # "Match-up Preview", rankings or age/record numbers from being captured
    # as player names.
    e1 = re.escape(p1)
    e2 = re.escape(p2)
    ...
    pair_patterns = [
        re.compile(
            rf"\b{e1}\s+and\s+{e2}\s+have\s+met\s+(?P<meetings>\d+)\s+times\b",
            flags=re.I,
        ),
        re.compile(
            rf"\b{e2}\s+and\s+{e1}\s+have\s+met\s+(?P<meetings>\d+)\s+times\b",
            flags=re.I,
        ),
    ]
    ...
    for rx in pair_patterns:
        pair_match = rx.search(text)
        if pair_match:
            break

    if not pair_match:
        print(f"TennisRatio VERIFIED: keine Paar-Zeile für {p1} vs {p2}")
        return None

    meetings = int(pair_match.group("meetings"))
    if meetings <= 0 or meetings > 30:
        print(f"TennisRatio VERIFIED reject: Meetings unplausibel {meetings}")
        return None

    # Only inspect a short segment AFTER the exact "have met N times" phrase.
    # This keeps unrelated records/rankings elsewhere on the page out.
    tail = text[pair_match.end():pair_match.end() + 600]

    leader_rx = re.compile(
        rf"\bwith\s+(?P<leader>{e1}|{e2})\s+leading\s+"
        rf"(?P<leadwins>\d+)\s*[-:–—]\s*(?P<trailwins>\d+)\b",
        flags=re.I,
    )
    leader_match = leader_rx.search(tail)

    if not leader_match:
        print(f"TennisRatio VERIFIED: keine Leader-Zeile für {p1} vs {p2}")
        return None

    leader = normalize_player_name(leader_match.group("leader"))
    lead_wins = int(leader_match.group("leadwins"))
    trail_wins = int(leader_match.group("trailwins"))

    # Triple consistency checks.
    if leader.casefold() not in {p1.casefold(), p2.casefold()}:
        print("TennisRatio VERIFIED reject: unbekannter Leader", leader)
        return None

    if lead_wins + trail_wins != meetings:
        print(
            "TennisRatio VERIFIED reject: Meetings/Siege inkonsistent",
            meetings, lead_wins, trail_wins
        )
        return None

    if lead_wins < trail_wins:
        print(
            "TennisRatio VERIFIED reject: 'leading' widerspricht Score",
            lead_wins, trail_wins
        )
        return None

    if leader.casefold() == p1.casefold():
        wins1, wins2 = lead_wins, trail_wins
    else:
        wins1, wins2 = trail_wins, lead_wins
    ...
    record = {
        ...
        "lastMeeting": None,
        "recentMeetings": [],
        "source": "TennisRatio H2H (verified exact-pair prose)",
        "sourceUrl": url,
    }

    print(
        f"TennisRatio VERIFIED OK: {p1} vs {p2} -> "
        f"{wins1}:{wins2} | meetings={meetings} | leader={leader}"
    )
    return record


def _run_h2h_parser_selftests():
    """
    Local regression tests. These run before production collection.
    If this parser ever starts accepting the old bogus 31:5/23:6 style
    numbers again, the workflow stops instead of poisoning h2h.json.
    """
    tests = [
        {
            "name": "Pegula-Shnaider exact",
            "page": """
                Jessica Pegula Age: 32 LAST 52 WEEKS 57 - 18 76.0% wins.
                Match-up Preview Diana Shnaider and Jessica Pegula have met
                4 times in professional competition, with Jessica Pegula
                leading 4-0. Their most recent encounter was in August 2026.
            """,
            "p1": "Jessica Pegula",
            "p2": "Diana Shnaider",
            "expected": (4, 0, 4),
        ...
        {
            "name": "Swiatek-Kostyuk exact",
            "page": """
                Iga Swiatek and Marta Kostyuk have met 4 times in professional
                competition, with Iga Swiatek leading 3-1. Other stats 31-5.
            """,
            "p1": "Marta Kostyuk",
            "p2": "Iga Swiatek",
            "expected": (1, 3, 4),
        ...
        {
            "name": "Reject unrelated numbers",
            "page": """
                Jessica Pegula 57-18. Diana Shnaider 32-23.
                Jessica Pegula and Diana Shnaider preview only. 31-5 23-6.
            """,
            "p1": "Jessica Pegula",
            "p2": "Diana Shnaider",
            "expected": None,
        ...
        {
            "name": "Reject inconsistent meetings",
            "page": """
                Diana Shnaider and Jessica Pegula have met 4 times in
                professional competition, with Jessica Pegula leading 4-2.
            """,
            "p1": "Jessica Pegula",
            "p2": "Diana Shnaider",
            "expected": None,
        },
        {
            ...
            "page": """
                Diana Shnaider and Jessica Pegula have met 4 times in
                professional competition, with Jessica Pegula leading 4–0.
            """,
            "p1": "Diana Shnaider",
            "p2": "Jessica Pegula",
            "expected": (0, 4, 4),
    ...
    for test in tests:
        result = _parse_tennisratio_h2h(
            test["page"], test["p1"], test["p2"], "selftest://tennisratio"
        )
        ...
        if expected is None:
            if result is not None:
                raise RuntimeError(
                    f"H2H SELFTEST FAIL [{test['name']}]: "
                    f"expected None, got {result}"
                )
            continue

        actual = (
            int(result.get("wins1") or 0),
            int(result.get("wins2") or 0),
            int(result.get("meetings") or 0),
        ) if result else None
    ...
    print("H2H PARSER SELFTEST: 5/5 OK")
...
def fetch_external_wta_h2h(player1, player2):
    ...
    for url in candidates:
        try:
            ...
            record = _parse_tennisratio_h2h(page, player1, player2, final_url)
            if record and int(record.get("meetings") or 0) > 0:
                print(
                    f"WTA H2H VERIFIED OK: {player1} vs {player2} -> "
...
def supplement_current_wta_h2h_web(h2h):
    ...
    for m in matches:
        ...
        # Additional safety: current H2H must remain internally plausible.
        if ext_n != int(external.get("wins1") or 0) + int(external.get("wins2") or 0):
            print("WTA H2H VERIFIED reject: Meeting-Summe inkonsistent", p1, "vs", p2)
            continue

        if ext_n > 30:
            print("WTA H2H VERIFIED reject: ungewöhnlich viele Meetings", p1, "vs", p2, ext_n)
            continue
    ...
    print("WTA H2H VERIFIED ergänzt/ersetzt:", changed)
    return h2h
...
def main():
    print("=" * 68)
    print("MISSION 1000 DATA ENGINE v6.3 VERIFIED H2H")
    print("RUNNING FILE: collector/fetch_intelligence_v6_3_verified_h2h.py")
    _run_h2h_parser_selftests()
    print("=" * 68)
    ...
    # Targeted sanity check for the pair that exposed the WTA H2H gap.
    ...
    if peg_shn:
        p1n = normalize_player_name(peg_shn.get("player1"))
        p2n = normalize_player_name(peg_shn.get("player2"))
        w1 = int(peg_shn.get("wins1") or 0)
        w2 = int(peg_shn.get("wins2") or 0)
        meetings = int(peg_shn.get("meetings") or 0)

        if p1n.casefold() == peg:
            peg_wins, shn_wins = w1, w2
        else:
            peg_wins, shn_wins = w2, w1

        print(
            "H2H VERIFIED CHECK Pegula/Shnaider:",
            f"Pegula {peg_wins}:{shn_wins} Shnaider",
            "| Meetings:", meetings,
            "| Source:", peg_shn.get("source")
        )
    else:
        print("H2H VERIFIED CHECK Pegula/Shnaider: NICHT GEFUNDEN")
    ...
    save_json(SOURCE / "rankings_fallback.json", {
        "generatedAt": now,
        "source": "Mission 1000 Data Engine v6.3 - latest observed match rankings",
        "players": ranking_fallback,
    })

    save_json(SOURCE / "players.json", {
        "generatedAt": now,
        "source": "Mission 1000 Data Engine v6.3",
        "players": players,
    })
    ...
    print("DATA ENGINE v6.3 VERIFIED H2H: OK")
