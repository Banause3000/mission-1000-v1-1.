...
USER_AGENT = "Mission1000-DataEngine/6.1-WTA-H2H-SAFE-DIAG"
...
def _parse_tennisratio_h2h(page, player1, player2, url):
    ...
    # DEBUG snippets for the pair that exposed the bad parser.
    if {p1.casefold(), p2.casefold()} == {
        "jessica pegula", "diana shnaider"
    }:
        lower = text.casefold()
        for token in ("have met", "leading", "head-to-head", "h2h"):
            pos = lower.find(token)
            if pos >= 0:
                snippet = text[max(0, pos-220):pos+520]
                print(f"TENNISRATIO DEBUG [{token}]:", snippet)

    # SAFE format observed on TennisRatio:
    ...
    rx = re.compile(
        ...
        r"(?P<meetings>\d+)\s+times\b.{0,220}?"
    ...
    m = rx.search(text)
    if not m:
        print(f"TennisRatio SAFE parser: keine explizite H2H-Zeile für {p1} vs {p2}")
        return None
    ...
    lead_wins = int(m.group("w1"))
    trail_wins = int(m.group("w2"))

    # Hard validation. If any relation fails, reject instead of inventing data.
    if {a.casefold(), b.casefold()} != {p1.casefold(), p2.casefold()}:
        print("TennisRatio SAFE reject: Spielernamen passen nicht", a, "|", b)
        return None

    if leader.casefold() not in {p1.casefold(), p2.casefold()}:
        print("TennisRatio SAFE reject: Leader passt nicht", leader)
        return None

    if lead_wins + trail_wins != meetings:
        print(
            "TennisRatio SAFE reject: Meetings != H2H Summe",
            meetings, lead_wins, trail_wins
        )
        return None

    if lead_wins < trail_wins:
        print("TennisRatio SAFE reject: 'leading' aber Score ist kleiner")
        return None
    ...
    print(
        f"TennisRatio SAFE parsed: {p1} vs {p2} -> "
...
def supplement_current_wta_h2h_web(h2h):
    ...
    for m in matches:
        ...
        if ext_n != int(external.get("wins1") or 0) + int(external.get("wins2") or 0):
            print("WTA H2H SAFE reject: Meeting-Summe inkonsistent", p1, "vs", p2)
            continue

        if ext_n > 30:
            print("WTA H2H SAFE reject: ungewöhnlich viele Meetings", p1, "vs", p2, ext_n)
            continue
...
def main():
    print("=" * 68)
    print("MISSION 1000 DATA ENGINE v6.1 WTA H2H SAFE DIAG")
    print("RUNNING FILE: collector/fetch_intelligence_v6_1_h2h_safe_diag.py")
    print("=" * 68)
    ...
    save_json(SOURCE / "rankings_fallback.json", {
        "generatedAt": now,
        "source": "Mission 1000 Data Engine v6.1 - latest observed match rankings",
        "players": ranking_fallback,
    })

    save_json(SOURCE / "players.json", {
        "generatedAt": now,
        "source": "Mission 1000 Data Engine v6.1",
        "players": players,
    })
    ...
    print("DATA ENGINE v6.1 WTA H2H SAFE DIAG: OK")
