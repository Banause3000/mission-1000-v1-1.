...
USER_AGENT = "Mission1000-DataEngine/3.3.2"
...
def _absolute_wta_url(value):
    value = html.unescape(str(value or "")).strip()
    if value.startswith("//"):
        return "https:" + value
    if value.startswith("/"):
        return "https://www.wtatennis.com" + value
    return value

def _fetch_bytes(url, timeout=35):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 Mission1000-EndpointDiscovery/3.3.2",
            "Accept-Language": "en-US,en;q=0.9",
            "Accept": "*/*",
...
def discover_pegula_shnaider_endpoint(directory):
    p1 = "Jessica Pegula"
    p2 = "Diana Shnaider"
    ...
    print("=" * 72)
    print("WTA ENDPOINT DISCOVERY v3.3.2: PEGULA vs SHNAIDER")
    print("=" * 72)
    print("Pegula Profil-ID:", id1 or "NICHT GEFUNDEN")
    print("Shnaider Profil-ID:", id2 or "NICHT GEFUNDEN")

    if not id1 or not id2:
        print("DISCOVERY STOP: Profil-ID fehlt.")
        return

    page_url = f"https://www.wtatennis.com/head-to-head/{id1}/{id2}"

    try:
        raw, status, final_url, content_type = _fetch_bytes(page_url)
        page = raw.decode("utf-8", errors="replace")
    except Exception as exc:
        print("H2H PAGE FEHLER:", type(exc).__name__, str(exc))
        return

    print("H2H Page:", page_url)
    print("HTTP Status:", status)
    print("Final URL:", final_url)
    print("Content-Type:", content_type)
    print("HTML Bytes:", len(raw))

    # 1) Look for endpoint-looking strings directly in HTML.
    patterns = [
        r'https?://[^"\'<>\s\\]+',
        r'["\']([^"\']*(?:api|graphql|head.?to.?head|h2h|player|match|stats)[^"\']*)["\']',
    ]
    candidates = set()
    for pattern in patterns:
        for match in re.findall(pattern, page, flags=re.I):
            ...
            if any(k in value.casefold() for k in
                   ("api", "graphql", "head-to-head", "headtohead", "h2h",
                    "player", "match", "stat")):
                candidates.add(value[:500])

    print()
    print("HTML Endpoint-Kandidaten:", len(candidates))
    for value in sorted(candidates)[:40]:
        print("  CANDIDATE:", value)

    # 2) Enumerate JS assets loaded by the page.
    assets = []
    for src in re.findall(r'<script[^>]+src=["\']([^"\']+)["\']', page, flags=re.I):
        url = _absolute_wta_url(src)
        if url.startswith("http") and url not in assets:
            assets.append(url)

    print()
    print("JavaScript Assets gefunden:", len(assets))
    for url in assets[:30]:
        print("  JS:", url)

    # 3) Download a bounded number of JS bundles and search their source for
    # route/API strings. This reveals calls made after the HTML shell loads.
    endpoint_hits = set()
    keyword_hits = []
    max_assets = min(len(assets), 24)

    for index, asset_url in enumerate(assets[:max_assets], 1):
        try:
            js_raw, js_status, js_final, js_type = _fetch_bytes(asset_url, timeout=25)
            if len(js_raw) > 6_000_000:
                print(f"JS {index}/{max_assets}: übersprungen >6MB:", asset_url)
                continue

            js = js_raw.decode("utf-8", errors="replace")
            lower = js.casefold()
            interesting = any(k in lower for k in
                              ("head-to-head", "headtohead", "h2h", "graphql",
                               "/api/", "playerstats", "matchstats"))

            print(
                f"JS {index}/{max_assets}: status={js_status} bytes={len(js_raw)} "
            ...
            # URLs and relative paths likely to be network endpoints.
            regexes = [
                r'https?://[^"\'`\s\\]+',
                r'["\'`](/[^"\'`]{2,240})["\'`]',
            ]
            for rx in regexes:
                for hit in re.findall(rx, js, flags=re.I):
                    hit = html.unescape(hit).replace("\\/", "/")
                    low = hit.casefold()
                    if any(k in low for k in
                           ("api", "graphql", "head-to-head", "headtohead",
                            "h2h", "player", "match", "stat")):
                        endpoint_hits.add(hit[:500])

            # Short context snippets around the most useful terms.
            for token in ("head-to-head", "headtohead", "graphql", "/api/",
                          "h2h", "matchstats", "playerstats"):
                pos = lower.find(token)
                if pos >= 0:
                    snippet = re.sub(r"\s+", " ", js[max(0, pos-220):pos+520])
                    keyword_hits.append((token, asset_url, snippet[:740]))

        except Exception as exc:
            print(f"JS {index}/{max_assets} FEHLER:", type(exc).__name__, str(exc))

    print()
    print("JS Endpoint-Kandidaten:", len(endpoint_hits))
    for value in sorted(endpoint_hits)[:80]:
        print("  ENDPOINT:", value)

    print()
    print("Relevante JS-Kontexte:", len(keyword_hits))
    for token, asset, snippet in keyword_hits[:24]:
        print("  TOKEN:", token)
        print("  ASSET:", asset)
        print("  CONTEXT:", snippet)

    # 4) Inspect embedded JSON top-level structure, not just match extraction.
    payloads = _json_payloads_from_html(page)
    print()
    print("JSON-Script-Payloads:", len(payloads))
    for idx, payload in enumerate(payloads, 1):
        if isinstance(payload, dict):
            print(f"  JSON {idx} Keys:", list(payload.keys())[:30])
        elif isinstance(payload, list):
            print(f"  JSON {idx}: list[{len(payload)}]")
        else:
            print(f"  JSON {idx}: {type(payload).__name__}")

    print("=" * 72)
    print("WTA ENDPOINT DISCOVERY ENDE")
    print("=" * 72)
...
def supplement_current_wta_h2h(h2h):
    ...
    for match in current:
        ...
        if directory is None:
            ...
            # TEMPORARY DISCOVERY: inspect the public WTA page and its JS
            # bundles to identify the endpoint used for dynamic H2H data.
            discover_pegula_shnaider_endpoint(directory)
...
def main():
    print("=" * 68)
    print("MISSION 1000 DATA ENGINE v3.3.2 ENDPOINT DISCOVERY")
    print("=" * 68)
